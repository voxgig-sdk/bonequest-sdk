...
local EpisodeEntity = {}
EpisodeEntity.__index = EpisodeEntity


function EpisodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EpisodeEntity)
  self._name = "episode"
  self._client = client
...
function EpisodeEntity:get_name()
  return self._name
end


function EpisodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EpisodeEntity.new(self._client, opts)
end
...
function EpisodeEntity:mark_deleted()
  self._deleted = true
end


function EpisodeEntity:deleted()
  return true == self._deleted
end


function EpisodeEntity:data_set(args)
  if args ~= nil then
...
function EpisodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EpisodeEntity:match_set(args)
  if args ~= nil then
...
function EpisodeEntity:match_get()
...
function EpisodeEntity:stream(action, args, callopts)
...
function EpisodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
