...
local QuoteEntity = {}
QuoteEntity.__index = QuoteEntity


function QuoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QuoteEntity)
  self._name = "quote"
  self._client = client
...
function QuoteEntity:get_name()
  return self._name
end


function QuoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QuoteEntity.new(self._client, opts)
end
...
function QuoteEntity:mark_deleted()
  self._deleted = true
end


function QuoteEntity:deleted()
  return true == self._deleted
end


function QuoteEntity:data_set(args)
  if args ~= nil then
...
function QuoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QuoteEntity:match_set(args)
  if args ~= nil then
...
function QuoteEntity:match_get()
...
function QuoteEntity:stream(action, args, callopts)
...
function QuoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
