...
The Lua SDK for the Bonequest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Episode(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Bonequest(nil):load(
  { id = "test01" }, nil
)
