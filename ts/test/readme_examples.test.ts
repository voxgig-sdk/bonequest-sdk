...
//     (\`new BonequestSDK(...)\` / \`BonequestSDK.test(...)\` become
//     \`BonequestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"episode":{"test01":{"id":"test01"}},"quote":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BonequestSDK(...)\` and any
// \`BonequestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BonequestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BonequestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
