...
class EpisodeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EpisodeEntity($this->_client, $opts);
    }

    /**
     * @param Episode|array $args Episode data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BonequestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Episode|array The current Episode data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Episode fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BonequestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Episode fields).
     */
     ...
     *
     * @param EpisodeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EpisodeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Episode|array The loaded Episode as an assoc-array at the
     *   SDK boundary; throws BonequestError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BonequestHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
