...
The PHP SDK for the Bonequest API. Provides an entity-oriented interface using PHP conventions.


## Install
```bash
composer require voxgig/bonequest-sdk
```
...
```php
[$result, $err] = $client->Episode(null)->load(["id" => "example_id"], null);
...
[$result, $err] = $client->Bonequest(null)->load(
    ["id" => "test01"], null
);
...
```
php/
├── bonequest_sdk.php          -- Main SDK class
...
The main class (`bonequest_sdk.php`) exports the SDK class
