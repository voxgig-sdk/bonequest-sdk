...
boru-driven command-line client **and** interactive REPL for the Bonequest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bonequest-cli)
make build
...
./bonequest-cli load 1 episode            # {id:1} shorthand
./bonequest-cli load '{id:1}' episode       # explicit match map

# 5. Override the API base URL for a single call
BONEQUEST_BASE=https://api.example.com ./bonequest-cli load 1 episode

# 6. No arguments -> interactive REPL
./bonequest-cli
bonequest> load 1 episode
bonequest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bonequest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bonequest-cli load 1 episode
   ```
...
```sh
./bonequest-cli load 1 episode          # scalar shorthand for {id:1}
./bonequest-cli load '{id:1}' episode     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bonequest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `episode`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bonequest-cli`. |
...
(`./bonequest-cli <expr>`) and interactively (the REPL), and expressions compose the
