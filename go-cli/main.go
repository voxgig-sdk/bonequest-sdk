// Command Bonequest-cli is a CLI + REPL for the Bonequest SDK,
...
const prompt = "bonequest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "episode quote search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BONEQUEST_APIKEY (api key), BONEQUEST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BONEQUEST_APIKEY carries the API key and
	// BONEQUEST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BONEQUEST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BONEQUEST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBonequestSDK(opts)
