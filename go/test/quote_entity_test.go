...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Quote(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Quote(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BONEQUEST_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		quoteRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.quote")))
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BONEQUEST_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BONEQUEST_TEST_QUOTE_ENTID"])
	...
	if env["BONEQUEST_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewBonequestSDK(core.ToMapAny(mergedOpts))
	}
