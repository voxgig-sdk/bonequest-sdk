...
import (
	...
	omni "github.com/voxgig-sdk/bonequest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BonequestError); is {
				out := map[string]any{
					"name":    "BonequestError",
...
func omniCtx(arg any, client *sdk.BonequestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BonequestSDK); is {
			client = live
