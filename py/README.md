...
The Python SDK for the Bonequest API. Provides an entity-oriented interface following Pythonic conventions.


## Install
```bash
pip install bonequest-sdk
```
...
```python
result, err = client.Episode(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Bonequest(None).load(
    {"id": "test01"}, None
)
