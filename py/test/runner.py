# Bonequest SDK test runner

from __future__ import annotations
import os
import json

from bonequest_sdk.utility.voxgig_struct import voxgig_struct as vs


class BonequestTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BonequestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BonequestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BonequestTestRunner.getenv("BONEQUEST_TEST_LIVE")
        override = BonequestTestRunner.getenv("BONEQUEST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BonequestTestRunner.getenv(key)
        ...
        explain = BonequestTestRunner.getenv("BONEQUEST_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BONEQUEST_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BonequestTestRunner._test_control is not None:
            return BonequestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BonequestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BonequestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BonequestTestRunner.load_test_control()
...
def load_env_local():
    BonequestTestRunner.load_env_local()


def env_override(m):
    return BonequestTestRunner.env_override(m)


def entity_data(v):
    return BonequestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BonequestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BonequestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BonequestTestRunner.load_test_control()


def live_client_options():
    return BonequestTestRunner.live_client_options()


def live_delay_ms():
    return BonequestTestRunner.live_delay_ms()
