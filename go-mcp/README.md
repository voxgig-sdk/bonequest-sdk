...
[MCP](https://modelcontextprotocol.io) server exposing the Bonequest SDK as
two agent tools — `bonequest_list` and `bonequest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bonequest-mcp)
make build

# 2. Provide credentials via the environment
export BONEQUEST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bonequest \
  -- /absolute/path/to/bonequest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bonequest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bonequest_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// bonequest_load: one record by id
{ "entity": "episode", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bonequest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bonequest \
     -- "$PWD"/dist/*/bonequest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bonequest_list` and `bonequest_load` tools now appear
   in new sessions. Ask the agent to *"list quote using bonequest"*
   and it calls `bonequest_list` with `{"entity":"quote"}`.
...
```sh
export BONEQUEST_APIKEY=sk_live_xxx            # API key
export BONEQUEST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bonequest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bonequest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BONEQUEST_APIKEY` | API key sent with every request. |
| `BONEQUEST_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

episode | quote | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./bonequest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bonequest_load","arguments":{"entity":"episode","query":{"id":1}}}}'
```
