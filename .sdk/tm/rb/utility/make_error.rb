...
module BonequestUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BonequestContext.new({}, nil)
    end
    op = ctx.op || BonequestOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BonequestResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BonequestError) ? err.msg : err.to_s
    ...
    sdk_err = BonequestError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BonequestError)
