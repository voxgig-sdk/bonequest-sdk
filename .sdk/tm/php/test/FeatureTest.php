...
class FtCtrl extends BonequestControl
{
...
class FtClient
{
    ...
    private BonequestUtility $utility;

    public function __construct(BonequestUtility $utility, array $options)
    {
    ...
    public function get_utility(): BonequestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BonequestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BonequestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BonequestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BonequestConfig::make_feature($name);
    ...
    private static function buildUrl(BonequestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BonequestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BonequestResponse($fetched);
        }
    ...
    private function fail(BonequestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BonequestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
