...
module BonequestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BonequestResponse.new({})
    ...
    if fetch_err
      ...
      response = BonequestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BonequestResponse.new(fetched)
    else
