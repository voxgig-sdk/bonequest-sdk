...
class QuoteDirectTest < Minitest::Test
  def test_direct_list_quote
    setup = quote_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-quote", setup[:live] ? "live" : "unit")
    if _should_skip
...
def quote_direct_setup(mockres)
