...
class QuoteEntity
  ...
  def make
    opts = @_entopts.dup
    QuoteEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BonequestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Quote, Hash] the current Quote data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BonequestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Quote fields)
  def match_get
  ...
  #
  # @param reqmatch [QuoteListMatch, Hash, nil] match filter (any subset of
  #   Quote fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Quote>, Array] the matching Quote items; raises BonequestError on failure
