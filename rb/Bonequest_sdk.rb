...
require_relative 'Bonequest_types'
...
class BonequestSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BonequestHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BonequestHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BonequestFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BonequestUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BonequestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BonequestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BonequestHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BonequestError.new(
        "#{op}_allow",
        "BonequestSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BonequestHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BonequestError.new(
        "graphql_error", "BonequestSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
